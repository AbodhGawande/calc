...
(function () {
  ...
  function money(v, cur) {
    const f = moneyFmts[cur] || (moneyFmts[cur] = new Intl.NumberFormat(cur === 'INR' ? 'en-IN' : 'en-US', { style: 'currency', currency: cur }));
    return f.format(v).replace('-', '−');
  }
  // Plain numbers for the ⇄ sheet's currency boxes: Indian grouping for INR (2,20,70,800), US for the rest (22,070,800).
  const plainFmts = { INR: new Intl.NumberFormat('en-IN', { maximumFractionDigits: 2 }), other: new Intl.NumberFormat('en-US', { maximumFractionDigits: 2 }) };
  const plainMoney = (v, cur) => (cur === 'INR' ? plainFmts.INR : plainFmts.other).format(Number(v.toPrecision(14))).replace('-', '−');
  ...
  window.Rates = { init, rates, age, rate, money, plainMoney, renderBar, refresh, busy: () => busy };
})();
